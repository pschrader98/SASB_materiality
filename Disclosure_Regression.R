...
data_financials <- data_financials %>%
  dplyr::filter(as.integer(substr(YearQuarter, 1, 4)) >= 2010 &
                  as.integer(substr(YearQuarter, 1, 4)) <= 2019)
...
# Select main regression variables to make working with data more manageable
data_reprisk = data_reprisk %>% dplyr::select(gvkey, YearQuarter, SICS.Codified.Industry, overall_manual_Material_words, overall_manual_NonMaterial_words,
                                              quarters_from_adoption, post_provisional_standard, Material_manual_earnings_overall_share,
                                              Material_manual_earnings_presentation_share, Material_manual_earnings_question_share, Material_manual_earnings_answer_share,
                                              NonMaterial_manual_earnings_overall_share, earnings_vol_20q, leverage_p1p99, roa_abs_p1p99, mtb_prevq_p1p99, altman_z_p1p99, 
                                              n_material, n_nonmaterial, severity_sum, reach_sum, novelty_sum
...
dw <- data_words %>%
  mutate(
    ...
    overall_manual_Material_words = as.numeric(overall_manual_Material_words)
  ) %>%
...
dict <- c(
  overall_manual_Material_sentences = "# of Material Words",
