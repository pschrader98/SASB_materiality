...
est_11 <- feols(
  n_incidents_t1_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
...
est_12 <- feols(
  n_incidents_t2_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
est_13 <- feols(
  n_incidents_t3_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
etable(
  ...
  file         = "./results/incidents_post_disclosure_words_ols.tex",
  replace      = TRUE,
  dict         = dict,
  digits       = "r3",
  digits.stats = "r3",
...
est_11 <- feols(
  n_car_5_material_t1_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
est_12 <- feols(
  n_car_5_material_t2_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
est_13 <- feols(
  n_car_5_material_t3_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
etable(
  ...
  file         = "./results/car_post_disclosure_words_ols.tex",
  replace      = TRUE,
  dict         = dict,
  digits       = "r3",
  digits.stats = "r3",
...
est_11 <- feols(
  n_reach_material_t1_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
est_12 <- feols(
  n_reach_material_t2_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
est_13 <- feols(
  n_reach_material_t3_log1p ~  post_provisional_standard + overall_material_disclosure_log1p + overall_material_disclosure_log1p:post_provisional_standard +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
  vcov = ~ cusip + YearQuarter
...
etable(
  ...
  file         = "./results/reach_post_disclosure_words_ols.tex",
  replace      = TRUE,
  dict         = dict,
  digits       = "r3",
  digits.stats = "r3",
...
est_es <- feols(
  n_incidents_t1_log1p ~
    i(quarters_from_adoption, overall_material_disclosure_log1p,
      ref = -1, keep = -12:12) +
    roa_abs_p1p99 + leverage_p1p99 + mtb_prevq_p1p99 + size_ln_mve_prevq
  | cusip + YearQuarter,
  data = data_post2010,
...
coefs <- tidy(est_es, conf.int = TRUE, conf.level = 0.95)
es_coefs <- coefs %>%
  filter(str_detect(term, "quarters_from_adoption.*overall_material_disclosure_log1p")) %>%
  mutate(k = as.integer(str_extract(term, "-?\\d+"))) %>%
  arrange(k)

ggplot(es_coefs, aes(x = k, y = estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +
  labs(
    x = "Quarters from adoption (ref = -1)",
    y = "Δ slope of log disclosure",
    title = "Event study: overall_material_disclosure_log1p × quarters_from_adoption"
  ) +
  theme_minimal()
